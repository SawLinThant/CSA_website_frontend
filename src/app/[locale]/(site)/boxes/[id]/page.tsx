...
export default async function BoxDetailPage({
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 pb-12 pt-6 md:pt-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.35fr_1fr]">
          <div className="relative min-h-[360px]">
            ...
            <div className="absolute bottom-6 left-6 right-6 text-white">
              ...
              <h1 className="text-balance text-5xl font-extrabold leading-[0.95] md:text-6xl">{box.name}</h1>
              {activeVersion ? (
                <p className="mt-2 text-sm text-white/90">
                  Version: {activeVersion.versionName}
                </p>
              ) : null}
            </div>
          </div>

          <div className="bg-[#f4f9ef] p-6 md:p-8">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Subscription Price</p>
            <div className="mt-1 flex items-end gap-1">
              <p className="text-5xl font-extrabold leading-none text-[#2f7a1f]">
            ...
            <div className="mt-6">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Select Frequency</p>
              <div className="mt-3 grid grid-cols-2 gap-2">
                <button
                  type="button"
                  className={`h-11 rounded-xl border px-3 py-2 text-sm font-semibold ${
                    activePlan?.deliveryFrequency === "weekly"
                      ? "border-[#2f7a1f] bg-[#2f7a1f] text-white"
                ...
                <button
                  type="button"
                  className={`h-11 rounded-xl border px-3 py-2 text-sm font-semibold ${
                    activePlan?.deliveryFrequency === "monthly"
                      ? "border-[#2f7a1f] bg-[#2f7a1f] text-white"
                      : "border-black/20 bg-white text-foreground"
                  }`}
                >
      ...
      <section className="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-[1.7fr_1fr]">
        <div>
          <h2 className="text-4xl font-bold text-foreground">Box Description</h2>
          <div className="mt-4 rounded-xl border border-black/10 bg-[#f8fbf5] p-3 text-sm text-foreground">
          ...
          <div className="mt-8">
            <div className="flex items-center justify-between gap-3">
              <div>
                <h3 className="text-4xl font-bold text-foreground">Sample Items</h3>
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-2xl border border-black/10 bg-[#f4f9ef] p-4 shadow-sm">
            <h3 className="text-lg font-semibold text-foreground">What's Included?</h3>
            <ul className="mt-3 space-y-2 text-sm text-foreground">
              <li className="flex items-start gap-2">
                <CheckCircle2 className="mt-0.5 size-4 text-[#2f7a1f]" />
                Seasonal produce from partner farms
              </li>
              <li className="flex items-start gap-2">
                <CheckCircle2 className="mt-0.5 size-4 text-[#2f7a1f]" />
                Weekly recipe guide
          ...
          <section className="rounded-2xl border border-black/10 bg-[#edf6e5] p-6 shadow-sm">
            ...
            <Link href={withLocalePath(locale, "/")} className="mt-3 block text-center text-sm font-semibold text-[#2f7a1f] hover:underline">
              Meet the Farmers
            </Link>
          </section>
        </aside>
